...
<body>
    ...
    <c:choose>
	  ...
	  <%-- 로그인이 되어 있지 않은 경우 --%>
		...
		<%--
			로그인 -> 아이디, 비밀번호 입력해서 -> LoginServlet / post 방식
		...
		<%--
			회원 검색 : 검색할 아이디를 입력받아서 -> FindMemberServlet -> 검색 성공하면 views/find_ok.jsp (정보 출력)
		...
		<%-- 로그아웃 : 로그아웃하고 index.jsp로 로면 됨 --%>
		
		<li><a href="views/update.jsp">회원정보 수정</a></li>
		<%-- 회원정보 수정 : 로그인한 정보가 기대된 상태로 시작 
		                ID값은 변경불가능하게 처리
