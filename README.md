...
- Tomcat : Apache Software Foundation에서 Servlet과 JSP 를 통한 동적인 웹 문서를 처리하기 위해 만든 웹 애플리케이션 서버
...
```java
public class 클래스명 extends HttpServler{
...
- javax.servlet.ServletReauest 를 상속
...
- setContentType(String) : 응답으로 작성하는 페이지의 MIME type을 정하는 메소든

## 서블릿 생명주기(Life Cycle)

- 서블릿 객체가 생성 후 소멸되기까지의 과정을 라이프 사이클이라고 한다.

1. 첫번재 요청일 경우, 서블릿 객체를 생성하고 init() 메소드를 호출한다.
   (단, 한번만 수행!)
...
3. open() 메소드 호출하여 요청방식, 대상(서버), 동기/비동기 지정
